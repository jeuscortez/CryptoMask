...
/**
 * Created by Cortez on 3/29/2017.
 */
...
public class Steganography {
     ...
     */
    public byte[] encodeMessage(int[] oneDPix, int imgCols, int imgRows, String str) {
        ...
        for (int row = 0; row < imgRows; row++) {
            for (int col = 0; col < imgCols; col++) {
                ...
                for (int channelIndex = 0; channelIndex < channels; channelIndex++) {
                    if (!msgEnded) {
                        tmp = (byte) ((((oneDPix[element] >> binary[channelIndex]) & 0xFF) & 0xFC) | ((message[msgIndex] >> toShift[(shiftIndex++)
     ...
     */
    public String decodeMessage(byte[] oneDPix, int imgCols,
                                int imgRows) {
